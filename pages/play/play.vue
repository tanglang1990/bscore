    ...
    <view class="game" v-if="playState != 3">
      <view class="head">
        ...
        <view class="nameInput" v-if="playState == 0">
          <input class="uni-input" placeholder="A方-姓名1" :value="players.a1" data-key="a1" @input='nameChange' />
          <input class="uni-input" placeholder="B方-姓名1" :value="players.b1" data-key="b1" @input='nameChange' />
          <input class="uni-input" placeholder="A方-姓名2" :value="players.a2" data-key="a2" @input='nameChange' />
          <input class="uni-input" placeholder="B方-姓名2" :value="players.b2" data-key="b2" @input='nameChange' />
        </view>
      </view>
      <view class="ground" v-if="playState == 1 || playState == 2">
        <view class="outline" v-if="side == 0">
          ...
          <view class="play center">
            <view class="player">
              <view class="name">
                <text>{{players.a2}}</text>
                <button type="default" size="mini" data-key="a2" @click="serveBall" v-if="serve.a == ''">{{ serve.w == '' ? '发球' : '接发球' }}</button>
              </view>
              <view class="icon" v-if="serve.a == 'a2' && serve.w == 'a'">
            ...
            <view></view>
            <view class="player">
              <view class="icon" v-if="serve.b == 'b1' && serve.w == 'b'">
                <image src="../../static/img/dance.png" mode="widthFix"></image>
              </view>
              <view class="name">
                <text>{{players.b1}}</text>
                <button type="default" size="mini" data-key="b1" @click="serveBall" v-if="serve.b == ''">{{ serve.w == '' ? '发球' : '接发球' }}</button>
              </view>
            </view>
          </view>
          <view class="play center">
            <view class="player">
              <view class="name">
                <text>{{players.a1}}</text>
                <button type="default" size="mini" data-key="a1" @click="serveBall" v-if="serve.a == ''">{{ serve.w == '' ? '发球' : '接发球' }}</button>
              </view>
              <view class="icon" v-if="serve.a == 'a1' && serve.w == 'a'">
                <image src="../../static/img/dance.png" mode="widthFix"></image>
              </view>
            </view>
            <view></view>
            <view></view>
            <view class="player">
              <view class="icon" v-if="serve.b == 'b2' && serve.w == 'b'">
                <image src="../../static/img/dance.png" mode="widthFix"></image>
              </view>
              <view class="name">
                <text>{{players.b2}}</text>
                <button type="default" size="mini" data-key="b2" @click="serveBall" v-if="serve.b == ''">{{ serve.w == '' ? '发球' : '接发球' }}</button>
              </view>
            </view>
          </view>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState('game', ['playState', 'players', 'result', 'detail']),
      ...mapGetters('game', ['detail']),
    },
    methods: {
      ...mapMutations('game', ['updatePlayer', 'updatePlayState', 'updateScore', 'startNext', 'playNewGame']),
      // 修改选手姓名
      ...
      serveBall: function(e) {
        ...
        var who = e.target.dataset.key // 'a1' 'a2' 'b1' 'b2'
        var side = who[0] // 'a' 'b'
        console.log(who, ' ', side)
        if (!this.serve.w) {
          this.serve.w = side
        }
        this.serve[side] = who
      ...
      // 开始比赛
      start: function() {
        if (!this.serve.w || !this.serve.b || !this.serve.a) {
        ...
        .item {
          ...
          .blue-line-left {
            ...
            :nth-child(1) {
              width: 60rpx;
              height: 100rpx;
              border: 2px solid rgb(77, 190, 240);
              border-left: none;
            }

            :nth-child(2) {
              border: 1px solid rgb(77, 190, 240);
              height: 0;
              width: 20rpx;
            }
          }
